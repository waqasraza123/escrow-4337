...
- The auth module now uses a provider-backed OTP email boundary with rollback-safe issuance, rotating refresh-token sessions with replay-triggered revocation, validated environment-driven JWT, session, and OTP runtime controls, persistence-backed per-IP OTP request throttles, and trusted-proxy-aware bootstrap configuration, but still needs live relay and deployed ingress validation.
