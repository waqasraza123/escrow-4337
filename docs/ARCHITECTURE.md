...
- The auth module now uses a provider-backed OTP email boundary with rollback-safe issuance, rotating refresh-token sessions with replay-triggered revocation, and validated environment-driven JWT, session, and OTP runtime controls, but still needs live relay validation and OTP abuse hardening.
