...
8. Preserve the generated launch-candidate artifact bundle for rollout evidence and any rollback review.
   The GitHub workflow now validates both the reviewed launch subset and the full `launch-candidate-*` upload bundle before publishing either artifact, so missing required launch files fail before promotion review ever consumes the bundle.
9. Run the manual `Promotion Review` workflow against the same environment using the candidate CI run id.
   ...
   The generated `release-dossier-*` and `release-pointer-*` bundles now also fail before upload if their canonical output files are incomplete.
