...
- `apps/web`: Next.js client and contractor product with a new marketing root plus trust page, route-scoped `/app` surfaces for sign-in, setup, contract creation, contract detail, delivery, and dispute, browser-wallet-native plus manual SIWE wallet-link challenge handling, smart-account provisioning, guided client job authoring, exact `workerAddress` binding in the contract composer, required contractor email capture for the pending participant identity, a copied join-link handoff for contractors, explicit contract-detail join posture that explains whether sign-in, matching email, or the bound worker wallet is still missing, a one-button contractor join action once those conditions are met, role-aware job workspaces, lifecycle mutation forms, audit visibility, and backend runtime-profile validation posture wired to the API. The dynamic contract routes now use Next 15 async `params` correctly, which unblocks browser-driven route testing.
...
- The launch-candidate join handoff is now a copied share-link plus exact worker-wallet binding and persisted pending contractor participation; there is still no automated invite delivery, wallet rebinding flow, or multi-contractor contract model.
...
- Frontend coverage now includes focused helper tests, route-level coverage in `apps/web` and `apps/admin`, a local-profile end-to-end browser flow, runtime-profile validation posture, one authenticated arbitrator-resolution path, and a new launch-candidate exact-flow harness for the create/fund/join/deliver/dispute/resolve path. The deployed version of that exact flow is wired but still depends on explicit staged `PLAYWRIGHT_DEPLOYED_FLOW_*` secrets and has not been proven end to end in this repo state.
