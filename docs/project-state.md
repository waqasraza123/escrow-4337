...
- Escrow operations health now surfaces cross-job attention from persisted API state, but it is still not backed by a separate indexing or replay pipeline and should not be treated as chain-reconciled truth yet.
