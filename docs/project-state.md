...
- Reconciliation findings should stay truthfully scoped to persisted API-side audit and execution history; replay improves drift detection and timeline validation, but it is not a chain indexer or source of onchain truth.
...
- Operations health now includes a replay-backed persisted-timeline reconciliation pass and a protected `job-history` import preview, but broader chain event ingestion and reconciliation against indexed onchain truth are still pending.
