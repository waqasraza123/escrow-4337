...
- Stale-job escalation beyond manual operator claims and a separate ingestion or reconciliation pipeline are still pending.
- Operations health is derived from persisted API-side escrow state, not from a separate chain indexer yet; broader event ingestion and reconciliation are still pending.
...
- Failed-execution diagnostics are intentionally bounded summaries over persisted API execution records; they improve operator triage but do not yet attach provider-native remediation guidance or chain reconciliation.
